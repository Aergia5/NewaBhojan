...
try {
    ...
    // For testing - set to false when using real database
    $useTestData = true;
    ...
    if ($useTestData) {
        ...
        if ($category !== 'all') {
            $testMenuItems = array_filter($testMenuItems, function($item) use ($category) {
        ...
        echo json_encode([
            'success' => true,
            'data' => array_values($testMenuItems) // reindex array after filtering
        ]);
        exit;
    }

    // Real database query (when $useTestData = false)
